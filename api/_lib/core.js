  ...
  const emitInline = (text) => {
    ...
    for (;;) {
      ...
      if (m || n || p) {
        ...
        /* schedule/pdf tags get their header immediately; notion result headers
           are written by the handler once the Notion API call resolves */
        if (m || p) writeHeader('\u0000action:' + JSON.stringify(action) + '\u0000');
